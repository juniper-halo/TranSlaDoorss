...
from core.base_model import BaseASLModel, EnsembleASLModel, ASLFeatureExtractor
from inference.asl_inference import ASLBenchmark, FewShotASL


class ASLEvaluator:
    """Comprehensive evaluator for ASL models"""
    
    def __init__(self, model: BaseASLModel, dataset_name: str = "aliciiavs/sign_language_image_dataset"):
        self.model = model
    ...
    def _analyze_per_letter_performance(self, num_samples: int) -> Dict[str, Dict[str, Any]]:
    ...
    def print_detailed_report(self, results: Dict[str, Any]):
        ...
        print(f"  Mean confidence (correct): {conf_analysis['mean_correct_confidence']:.3f}")
        print(f"  Mean confidence (incorrect): {conf_analysis['mean_incorrect_confidence']:.3f}")
        ...
        for letter in sorted(per_letter.keys()):
            ...
            print(f"  {letter}: {stats['accuracy']:.2%} ({stats['correct_predictions']}/{stats['total_samples']})")
        
        print(f"\nDifficult Samples (top 5):")
        difficult = detailed['difficult_samples'][:5]
        for i, sample in enumerate(difficult):
            print(f"  {i+1}. True: {sample['true_letter']}, Pred: {sample['pred_letter']}, "
...
class ModelComparison:
    ...
    def __init__(self, models: List[BaseASLModel], dataset_name: str = "aliciiavs/sign_language_image_dataset"):
        self.models = models
        self.dataset_name = dataset_name
        self.dataset = None
