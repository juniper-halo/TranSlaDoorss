...
class CLIPInferenceBenchmark:
            ...
            try:
                ...
                if "img" in example:  # CIFAR-10 uses 'img'
                    ...
                    for _, value in example.items():
                        ...
                        print(
                            f"No image found in example {i}, keys: {list(example.keys())}"
                        )
                        continue
                ...
                print(
                    f"Example keys: {list(example.keys()) if hasattr(example, 'keys') else 'No keys'}"
                )
                continue
    ...
    def run_complete_benchmark(self, dataset_name="cifar10", num_samples=500):
        ...
        if not processed_data:
            print(
                "No images were successfully processed. Trying alternative approach..."
            )
...
def main():
    ...
    parser.add_argument(
        "--dataset", type=str, default="cifar10", help="HuggingFace dataset name"
    )
    ...
    benchmark = CLIPInferenceBenchmark(model_name=args.model, device=args.device)
