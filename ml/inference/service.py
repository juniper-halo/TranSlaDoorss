...
- warm the cache on startup (recommended) by calling get_predictor() once in your AppConfig.ready():
    from ml.inference.service import get_predictor
...
- python -m ml.inference.service --model-id ml/saved_weights/epoch_7 --image /path/to/image.jpg --top-k 3
...
def resolve_model_id(model_id: str | None = None) -> str:
    """
    Resolve the model id/path with the following precedence:
    1) explicit arg
    2) env ASL_MODEL_ID
    3) best_checkpoint.json (ml/saved_weights)
    4) fallback base CLIP
    """
    if model_id:
        return model_id
    from_env = os.environ.get("ASL_MODEL_ID")
    if from_env:
        return from_env
    best = resolve_best_checkpoint_model_id(project_root=PROJECT_ROOT)
    return best or FALLBACK_MODEL_ID
