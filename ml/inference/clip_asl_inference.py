...
- inference service (ml.inference.service.get_predictor / django)
- testing/evaluator (ml.testing.evaluator)
...
class ASLPredictor(BaseASLModel):
    ...
    def __init__(self, model_name: Optional[str] = None, device: Optional[str] = None):
        ...
        Args:
            model_name: model id/path to load (hf repo id or local checkpoint); defaults to ASL_MODEL_ID, then best_checkpoint.json, then base CLIP.
            device: optional torch device string. defaults to "cuda" if available.
        """
        if model_name is None:
            project_root = Path(__file__).resolve().parents[2]
            model_name = (
                os.environ.get("ASL_MODEL_ID")
    ...
    def predict_with_probs(self, image: Image.Image) -> Tuple[str, float, torch.Tensor]:
        """
        Predict ASL letter and return the full probability vector.

        Returns:
            (predicted_letter, confidence_score, probs) where probs is shape [1, num_letters]
        """
        processed_image = self.preprocessor.preprocess(image)
