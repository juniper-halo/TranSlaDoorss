...
from ml_dev.inference.load_best_checkpoint import get_best_checkpoint_path  # noqa: E402
...
class TranslatorView(APIView):
    ...
    def post(self, request: Request):
        ...
        try:
            try: 
                best_checkpoint = get_best_checkpoint_path(PROJECT_ROOT / "ml_dev" / "saved_weights")
            except Exception as e:
                print(f"Could not load best checkpoint path: {e}. Falling back to default model.")
...
class FeedbackSerializer(serializers.Serializer):
    ...
    def validate_predicted_label(self, value):
        if not value.isupper() or not value.isalpha():
            raise serializers.ValidationError("Predicted label must be a single uppercase letter (A-Z)")
        return value
    
    def validate_correct_label(self, value):
        if not value.isupper() or not value.isalpha():
            raise serializers.ValidationError("Correct label must be a single uppercase letter (A-Z)")
        return value
